...
When deployed on Vercel (Auth Service), if AI_ENGINE_URL is set (e.g.
https://ai-tutor-api.onrender.com), any AI/Compute request is proxied
transparently to the Render AI Engine.
"""
...
async def proxy_ai_request(request: Request, path: str):
    ...
    if not AI_ENGINE_URL:
        return Response(
            content='{"detail": "AI Engine URL (AI_ENGINE_URL) is not configured on Vercel."}',
            status_code=503,
            media_type="application/json",
        )

    target_url = f"{AI_ENGINE_URL}/{path}"
    ...
    try:
        ...
        return Response(
            content=f'{{"detail": "AI Engine communication error: {str(e)}"}}',
            status_code=502,
            media_type="application/json",
        )
